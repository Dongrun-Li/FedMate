...
class LocalUpdate_FedAvg(object):
    def __init__(self, idx, args, train_set, test_set, model, h_l):
        ...
        self.agg_weight = torch.tensor(len(train_set.dataset)).to(self.device)
        self.h_l = h_l
    ...
    def update_local_model(self, global_weight):
        """Update entire local model with global weights"""
        self.local_model.load_state_dict(global_weight)

    def _train_epochs(self, optimizer, num_epochs, classifier_only=False):
    ...
    def local_fine_tuning(self, local_epoch, round=0):
        """Fine-tune only classifier layers"""
        self.local_model.train()
        acc1 = self.local_test(self.test_data)
        ...
        # Setup optimizer for classifier parameters only
        optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, self.local_model.parameters()),
                                   lr=self.args.lr, momentum=0.5, weight_decay=0.0005)
        
        # Train model
        iter_loss = self._train_epochs(optimizer, local_epoch, classifier_only=True)
